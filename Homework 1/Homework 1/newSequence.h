...
typedef unsigned long ItemType; // #2

const int DEFAULT_MAX_ITEMS = 200;

class Sequence {
public:
    Sequence(int x);    // Create an empty sequence (i.e., one whose size() is 0).
    Sequence();
    Sequence(const Sequence& copy);
    ~Sequence();
    Sequence& operator = (const Sequence& z);
