...
void Rat::move()
{
    ...
    if (attemptMove(*m_arena, randInt(0, NUMDIRS-1), m_row, m_col))
    {
        ...
        {
            if (m_health == INITIAL_RAT_HEALTH){ // When an unpoisoned rat eats a poison pellet, its arena's history object must be notified about the position where it ate the poison pellet
                (*m_arena).history().record(m_row, m_col); // added
            }
